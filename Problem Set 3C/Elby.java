public class Elby {
    ...
    public String getResponse(String statement) {
        String response = "";
        if(findKeyword(statement, "dog",0)>=0||findKeyword(statement, "cat",0)>=0){
        ...
        else if(findKeyword(statement, "chris",0)>=0){
            response="Chris is a funny fella";
        }else if(statement.trim().length()==0){
            response= "say something";
        }else if(findKeyword(statement, "feller",0)>=0){
            response= "you're the fella not me";
        }else if(findKeyword(statement, "phone",0)>=0){
            response="oh you think the phone is better than me?";
        }else if(findKeyword(statement, "sarcastic",0)>=0){
            response="im not sarcastic, you are";
        }else if(findKeyword(statement,"you",0)<findKeyword(statement,"me",0)&&findKeyword(statement,"you",0)>=0){
            response=transformYouMeStatement(statement);
        }else if(findKeyword(statement,"i want to",0)>=0&&statement.length()>findKeyword(statement, "i want to",0)+10){
            response=transformIWantToStatement(statement);
        }else if(findKeyword(statement, "i want",0)>=0&&statement.length()>findKeyword(statement, "i want",0)+7){
            response=transformIWantStatement(statement);
        }else if(findKeyword(statement,"i",0)<findKeyword(statement,"you",0)&&findKeyword(statement,"i",0)>=0){
    ...
    public String transformIWantStatement(String statement){
        ...
        int index= findKeyword(statement,"I want",0);
        String restOfStatement=statement.substring(index+6).trim();
    ...
    public String transformIMeStatement(String statement) {
        ...
        int posOfYou  = findKeyword(statement, "you",  posOfI + 2);
